...
namespace BankSystem
{
    ...
    {
        private static bool NameValidate(string str)
        {
        ...
        private static bool EmailValidate(string str)
        {
        ...
        private static bool DOBValidate(string str)
        {
        ...
        private static bool ContactValidate(string str)
        {
            if (Regex.Match(str, "\\A[0-9]{10}\\z").Success)
                return false;
            else return true;
        }
        private static bool GenderValidate(string str)
        {
            if (Regex.Match(str, "(?:m|M|male|Male|f|F|female|Female|FEMALE|MALE)$").Success)
                return false;
            else return true;
        }
        private static bool MaritalStatusValidate(string str)
        {
            if (Regex.Match(str, "(?:y|n|yes|no|Yes|No|YES|NO)$").Success)
                return false;
            else return true;
        }
        private static bool PANCardValidate(string str)
        {
        ...
        {
            String firstName, lastName, email, contact, gender, dob;
            String spouseFirstName="", spouseLastName="", spouseContact;
            String childrenStatus;
            String[] childrenName;
            int childCount;
            String maritalStatus, accountTypeChoice;
            String accountType, panNumber, permAddress, commAddress;
            string sameAddressChoice, creditCardChoice;
            int monthlyIncome, creditCardTypeChoice;
            string creditCardType;
            string[] creditCardTypes = {"Silver", "Gold", "Platinum"};
            string creditCardNumber = "", debitCardNumber = "", accountNumber = "";
            string[] existingCreditCardNumbers = new string[5];
            string[] existingDebitCardNumbers = new string[5];
            string[] existingAccountNumbers = new string[5];
            ...
            if (creditCardChoice == "yes" || creditCardChoice == "y")
            {
                ...
                {
                    Console.WriteLine("You entered an invalid amount..Exiting");
                    Environment.Exit(0);
                }
                else
                {
